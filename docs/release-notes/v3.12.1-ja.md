...
> 安定性修正、StepFun プリセット、OpenClaw authHeader 対応、新プロバイダー
...
CC Switch v3.12.1 は、安定性の改善とバグ修正に焦点を当てたパッチリリースです。共通設定モーダルの無限再オープンループ、WebDAV 同期時の外部キー制約エラー、複数の i18n 補間問題を修正しました。また、**StepFun（阶跃星辰）** プロバイダープリセットの追加、OpenClaw の**入力タイプ選択**と **authHeader** サポート、デフォルト Gemini モデルの **3.1-pro** へのアップグレード、4 つの新プロバイダーの追加が含まれます。
...
- **新プロバイダー**: Micu API、XCodeAPI、SiliconFlow、CTok をプロバイダーとして追加
...
- **Micu API**: Micu API をプロバイダーとして追加、アフィリエイトリンク付き
- **XCodeAPI**: XCodeAPI をプロバイダーとして追加
- **SiliconFlow**: SiliconFlow（硅基流动）をプロバイダーとして追加、アフィリエイトリンク付き
- **CTok**: CTok をプロバイダーとして追加
...
- **Compshare リンク**: Compshare プロバイダー登録リンクを coding-plan ページに更新
...
[Releases](https://github.com/tanaer/cc-switch/releases/latest) から適切なバージョンをダウンロードしてください。
