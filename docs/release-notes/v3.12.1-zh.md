...
> 稳定性修复、StepFun 预设、OpenClaw authHeader 支持，以及新服务商
...
CC Switch v3.12.1 是一个以稳定性改进和 Bug 修复为主的补丁版本。修复了通用配置弹窗无限重复打开的循环问题、WebDAV 同步时的外键约束失败以及多个 i18n 插值问题。同时新增了 **StepFun（阶跃星辰）** 供应商预设、OpenClaw **输入类型选择** 和 **authHeader** 支持，将默认 Gemini 模型升级到 **3.1-pro**，并欢迎四位新服务商加入。
...
- **新服务商**：欢迎 Micu API、XCodeAPI、SiliconFlow、CTok 加入服务商
...
- **Micu API**：新增 Micu API 服务商及推广链接
- **XCodeAPI**：新增 XCodeAPI 服务商
- **SiliconFlow**：新增 SiliconFlow（硅基流动）服务商及推广链接
- **CTok**：新增 CTok 服务商
...
- **Compshare 链接**：更新 Compshare 服务商注册链接指向 coding-plan 页面
...
访问 [Releases](https://github.com/tanaer/cc-switch/releases/latest) 下载对应版本。
